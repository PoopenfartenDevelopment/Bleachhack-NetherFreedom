...
public class ModuleClickGuiScreen extends ClickGuiScreen {
	...
	public void initWindows() {
		...
		for (ModuleCategory c: ModuleCategory.values()) {
			addWindow(new ModuleWindow(ModuleManager.getModulesInCat(c), x, 50, len, StringUtils.capitalize(c.name().toLowerCase()), c.getItem()));
			x += len + 10;
		}
	...
	public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
		...
		textRenderer.draw(matrices, "BleachHack-NetherFreedom" + BleachHack.VERSION + "-" + SharedConstants.getGameVersion().getName(), 3, 3, 0x305090);
		textRenderer.draw(matrices, "BleachHack-NetherFreedom" + BleachHack.VERSION + "-" + SharedConstants.getGameVersion().getName(), 2, 2, 0x6090d0);
